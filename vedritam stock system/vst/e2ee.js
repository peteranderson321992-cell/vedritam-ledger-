        ...
        if (file && file.bytes) {
          ...
          jobs.push(subtle.encrypt({ name: 'AES-GCM', iv: fileIv }, msgKey, file.bytes)
            .then(function (ct) {
              ...
              return subtle.digest('SHA-256', attBytes).then(function (digest) {
                env.file = { name: file.name, type: file.type || '', iv: b64(fileIv),
                  size: file.bytes.byteLength, sha256: b64(digest) };
              });
            }));
        }
