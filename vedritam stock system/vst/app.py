...
# The frontend is served from the same origin as the API, so CORS only matters
# for external clients. Authentication uses a Bearer token (not cookies), so a
# wildcard origin without credentials is both valid and safe here.
