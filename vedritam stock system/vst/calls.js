...
(function () {
  var ICE = {
    iceServers: [
      ...
      // Free public relay: used only when the two devices cannot reach each
      // other directly (different networks, strict office firewalls).
  ...
  var remote = {};       // peer -> the stream we are receiving from them
  var pending = {};      // peer -> queued ICE candidates (before the answer/offer lands)
  var polite = {};       // peer -> negotiation state (polite side, offer in flight)
  ...
  function playSafely(el) {
    var go = function () { try { el.play().catch(function () {}); } catch (e) {} };
    go();
    // Some browsers refuse to start media until the page is touched again.
    document.addEventListener('click', go, { once: true });
  }

  function liveVideo(stream) {
    if (!stream) return false;
    return stream.getVideoTracks().some(function (t) {
  ...
  function tile(name, stream, isMe) {
    ...
    // Sound always comes out of a dedicated audio element, so a picture that
    // arrives (or stops) later never takes the voice with it.
    if (stream && !isMe) {
    ...
    if (liveVideo(stream)) {
      ...
      video.muted = true;               // never echo: the audio element has the sound
      playSafely(video);
      if (faceEl) faceEl.style.display = 'none';
    } else {
      // Voice only: drop any stale picture and show the avatar again.
  ...
  /* Perfect negotiation: both sides may offer at the same moment (each one
     creates its peer connection the instant it hears about the other). The
     "polite" side — decided the same way on both machines, by comparing the
     two user names — gives way when that happens, so exactly one offer wins
     and the media actually flows instead of both sides stalling. */
  function isPolite(name) {
    return String(me).toLowerCase() < String(name).toLowerCase();
  }
  ...
  function peer(name) {
    ...
    polite[name] = { polite: isPolite(name), making: false, ignoring: false, tries: 0 };

    // Always publish one audio and one video slot, even on a voice call: the
    // video slot stays empty until somebody shares a screen or a camera, and
    // having it up front means that switch needs no fragile renegotiation.
    if (local) local.getTracks().forEach(function (t) { pc.addTrack(t, local); });
    try {
      var kinds = pc.getTransceivers().map(function (t) {
        return (t.sender && t.sender.track && t.sender.track.kind) || '';
      });
      if (kinds.indexOf('audio') < 0) pc.addTransceiver('audio', { direction: 'sendrecv' });
      if (kinds.indexOf('video') < 0) pc.addTransceiver('video', { direction: 'sendrecv' });
    } catch (e) {}
    ...
    pc.ontrack = function (e) {
      ...
      remote[name] = stream;
      tile(name, stream, false);
      var refresh = function () { tile(name, stream, false); };
    ...
    pc.onnegotiationneeded = function () {
      ...
      Promise.resolve()
        ...
        .catch(function () {})
        .then(function () { st.making = false; });
    };
    pc.oniceconnectionstatechange = function () {
      var st = polite[name];
      if (pc.iceConnectionState === 'failed' && st && st.tries < 2) {
        // A dropped path is usually recoverable: gather fresh candidates
        // instead of tearing the whole call down.
        st.tries += 1;
    ...
    pc.onconnectionstatechange = function () {
      ...
      if (pc.connectionState === 'closed') dropPeer(name);
    };
    return pc;
  }
  ...
  function callPeer(name) {
    ...
    // and sends the offer. Doing it twice used to collide with itself.
    return peer(name);
  }
  ...
  function handleSignal(sig) {
    ...
    if (sig.kind === 'offer' || sig.kind === 'answer') {
      var desc = sig.payload;
      if (!desc || !desc.type) return;
      var collision = desc.type === 'offer' && (st.making || pc.signalingState !== 'stable');
      st.ignoring = !st.polite && collision;
      if (st.ignoring) return;                    // the other side will back off

      var ready = (collision && st.polite)
        ? Promise.all([
      ...
      ready
        .then(function () { return pc.setRemoteDescription(new RTCSessionDescription(desc)); })
        .then(function () {
          flushIce(from, pc);
  ...
  function hangup(notify) {
    ...
    peers = {}; tiles = {}; pending = {}; polite = {}; remote = {};
  ...
  function replaceOutgoingVideo(track) {
    Object.keys(peers).forEach(function (n) {
      var pc = peers[n];
      // Every connection already carries a video slot (see peer()), so the
      // picture can be swapped in and out without a new round of signalling.
      var tr = pc.getTransceivers().filter(function (t) {
        var k = (t.sender && t.sender.track && t.sender.track.kind) ||
                (t.receiver && t.receiver.track && t.receiver.track.kind) || '';
        return k === 'video';
      })[0];
      var sender = tr ? tr.sender
        : pc.getSenders().filter(function (s) { return s.track && s.track.kind === 'video'; })[0];
      if (sender) {
        if (tr && track && tr.direction === 'recvonly') tr.direction = 'sendrecv';
        sender.replaceTrack(track).catch(function () {});
      } else if (track) {
        // No video slot at all (older browser): add one and renegotiate.
  ...
  function toggleShare() {
    ...
    askScreen().then(function (ok) {
      ...
      navigator.mediaDevices.getDisplayMedia({ video: true, audio: false }).then(function (s) {
        ...
        // Keep the local preview on the camera stream; do not render the display-capture stream locally.
        // This avoids the common recursive mirror when the user shares the whole screen containing the call UI.
        tile(me, local, true);
