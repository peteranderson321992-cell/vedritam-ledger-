...
class Identity:
    ...
    def creatable_roles(self) -> List[str]:
        ...
        if self.is_admin:
            # Only the Super Admin may hand out the Admin role.
            return [database.STAFF, database.USER]
        if self.is_staff:
    ...
    def assert_can_manage_user(self, target: Dict[str, Any]) -> None:
        ...
        if uname.lower() == self.username.lower():
            # A non-Super-Admin may not alter their own privilege, status, or
            # school assignment through the administrative user API.
            raise HTTPException(status_code=403, detail="You cannot modify your own administrative account.")
        if not (self.is_admin or self.is_staff):
...
def get_identity(credentials: HTTPAuthorizationCredentials = Depends(security)) -> Identity:
    ...
    if security_mod.session_expired(username, float(payload.get("iat", 0) or 0)):
        try:
...
def login(request: LoginRequest, http_request: Request = None):
    ...
    # A locked account with enrolled 2FA gets a secure recovery path:
    # correct password + valid 2FA code can sign in immediately. Without the
    # second factor, the user must wait for the 15-minute lockout to expire.
    # This keeps the lockout useful while avoiding the old dead-end where 2FA
    # could never be reached once the password threshold was hit.
    if locked:
        if record and record.get("status") == "Active" and utils.verify_password(
                request.password, record.get("password_hash", "")):
            if security_mod.twofa_required(record["username"]):
                if not request.code:
                    security_mod.record_login(
                        record["username"], "2FA_REQUIRED", ip, agent,
                        "Account locked; 2FA bypass offered")
                    return {
                        "status": "2fa_required",
                        "reason": "lockout",
                        "lockout_seconds": locked,
                        "message": (
                            "Too many failed password attempts. Enter your 2FA code "
                            "to sign in now, or wait %d minute(s) and try again."
                            % max(1, (locked + 59) // 60)
                        ),
                    }
                if not security_mod.verify_totp(record["username"], request.code):
                    security_mod.record_login(record["username"], "2FA_FAILED", ip, agent,
                                              "Bad code during lockout")
                    raise HTTPException(status_code=401,
                                        detail="That verification code is not valid. You can also wait for the lockout to expire.")
                security_mod.clear_failures(request.username)
                # Continue through the normal successful-login/session path.
            else:
    ...
    if not utils.verify_password(request.password, record.get('password_hash', '')):
        ...
        locked_now = security_mod.check_lockout(request.username)
        if locked_now:
            database.log_action(record['username'], record.get('role', ''), "LOCKOUT", "auth", record['username'], "Account locked after failed password threshold")
            raise HTTPException(status_code=429,
                                detail=f"Account locked. Try again in {locked_now // 60 + 1} minute(s).")
        raise HTTPException(status_code=401,
    ...
    # Single active device: the person already signed in keeps their session.
    # A second sign-in is refused instead of taking the account over.
    if security_mod.device_session_live(user["username"]):
        info = security_mod.active_device_info(user["username"])
        since = info.get("started", "")
        security_mod.record_login(user['username'], "BLOCKED_ACTIVE_SESSION", ip, agent,
                                  f"Already signed in since {since}")
        database.log_action(user['username'], user['role'], "SESSION_BLOCKED", "auth",
                            user['username'], "Sign-in refused: account already active on another device")
        raise HTTPException(
            status_code=409,
            detail=("This account is already signed in on another device"
                    + (f" (since {since})" if since else "")
                    + ". Sign out there first, or wait "
                    f"{security_mod.get_idle_minutes()} minutes of inactivity, and try again."),
        )
...
def signup(request: SignupRequest, http_request: Request):
    if not security_mod.signup_allowed(http_request.client.host if http_request.client else "unknown"):
...
@router.get("/schools/assignment-options")
def school_assignment_options(identity: Identity = Depends(get_identity)):
    identity.assert_can_manage_schools()
    schools = database.get_all_schools()
    users = [u for u in database.get_all_users()
             if database.normalize_role(u.get("role","")) != database.SUPER_ADMIN
...
def remove_ledger_field(key: str, identity: Identity = Depends(get_identity)):
    ...
    if not database.delete_ledger_custom_field(key, identity.username):
...
def _sanitize_ledger_custom_data(rows: List[Dict[str, Any]], identity: Identity) -> List[Dict[str, Any]]:
    """Expose custom ledger values only for fields owned by the current user.
    A privileged caller may explicitly request another user's fields only via
    a future audited admin workflow; there is no implicit global schema."""
    allowed = {f.get("key") for f in database.get_ledger_custom_fields(identity.username)}
    out = []
    for row in rows:
        r = dict(row)
        raw = r.get("custom_json")
        if isinstance(raw, str):
            try: raw = json.loads(raw or "{}")
            except Exception: raw = {}
        if not isinstance(raw, dict): raw = {}
        r["custom_json"] = {k:v for k,v in raw.items() if k in allowed}
        out.append(r)
    return out
...
def get_ledger_for_class(school_id: int, class_id: int, identity: Identity = Depends(get_identity)):
    ...
    return {
        ...
        "rows": _sanitize_ledger_custom_data(database.get_ledger_records(school_id, class_id, created_by=identity.owner_filter), identity),
    }
...
def get_ledger(school_id: int, class_id: int, identity: Identity = Depends(get_identity)):
    identity.assert_school(school_id)
    return _sanitize_ledger_custom_data(database.get_ledger_records(school_id, class_id, created_by=identity.owner_filter), identity)
...
def _can_decide_transfer(identity: "Identity", record: Dict[str, Any]) -> bool:
    """Only the receiving school's Admin may accept/decline.

    The person who created the request must never see decision buttons, even
    when they are also assigned to the destination school.
    """
    if str(record.get("created_by", "")).lower() == identity.username.lower():
        return False
    if identity.is_super_admin:
        return True
...
@router.get("/transfers/schools")
def get_transfer_schools(identity: Identity = Depends(get_identity)):
    """School directory used only by the transfer workflow.

    The source list is restricted to schools assigned to the Admin. The
    destination list can contain any active school because a transfer is
    specifically a cross-school workflow; the server still requires an active
    receiving Admin before the request is created.
    """
    if not (identity.is_super_admin or identity.is_admin):
        raise HTTPException(status_code=403, detail="Only a school Admin can manage transfers.")
    all_schools = database.get_all_schools()
    active = [s for s in all_schools if str(s.get("status", "Active")) == "Active"]
    if identity.is_super_admin:
        assigned = active
    else:
        allowed = {str(i) for i in (identity.school_ids or [])}
        assigned = [s for s in active if str(s.get("id")) in allowed]
    return {"assigned": assigned, "all": active}
...
def get_transfers(identity: Identity = Depends(get_identity)):
    ...
    rows = modules.list_transfers(identity.scope_school_id,
                                  ...
                                  created_by="")
    for row in rows:
        # Explicitly expose whether this row was created by the signed-in user
        # so the client can render Pending instead of decision controls.
        row["is_sender"] = str(row.get("created_by", "")).lower() == identity.username.lower()
...
def post_transfer(request: TransferCreate, identity: Identity = Depends(get_identity)):
    # Transfers are a school-to-school administrative workflow. Only an Admin
    # assigned to the source school (or the Super Admin) may create one.
    if not (identity.is_super_admin or identity.is_admin):
        raise HTTPException(status_code=403, detail="Only a school Admin can send transfer requests.")
    identity.assert_school(request.fromSchoolId)
    identity.assert_school(request.toSchoolId) if identity.is_super_admin else None

    destination_admins = database.admin_usernames_for_school(request.toSchoolId)
    if not destination_admins:
        raise HTTPException(status_code=400,
                            detail="The destination school has no active Admin assigned to receive this transfer.")
    ...
    # Route a notification to the receiving school's Admin(s). Do not notify
    # the sender as the recipient of their own request.
    school_names = {str(s.get("id")): s.get("name", "") for s in database.get_all_schools()}
    from_name = school_names.get(str(request.fromSchoolId), "source school")
    to_name = school_names.get(str(request.toSchoolId), "destination school")
    if destination_admins:
        messaging.broadcast_notification(
            destination_admins, "transfer", "New transfer request",
            f"{request.quantity} × {request.bookName} requested from {from_name} to {to_name}.",
            "transfers.html"
        )
...
def put_transfer_status(transfer_id: str, request: TransferStatusUpdate,
                        identity: Identity = Depends(get_identity)):
    """Only an Admin of the receiving school can accept/decline."""
    existing = modules.get_transfer(transfer_id)
    if not existing:
        raise HTTPException(status_code=404, detail="Transfer not found.")
    if not _can_decide_transfer(identity, existing):
        raise HTTPException(status_code=403,
                            detail="Only an Admin of the receiving school can decide this transfer.")
    decision = {"Approved": "Accepted", "Rejected": "Declined"}.get(request.status, request.status)
    if decision == "Declined" and not (request.remarks or "").strip():
        raise HTTPException(status_code=400, detail="A reason is required when declining a transfer.")
    try:
        rec = modules.set_transfer_status(transfer_id, decision, identity.username,
                                          (request.remarks or "").strip())
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))

    # Notify the original sender with the final decision and remarks.
    sender = str(existing.get("created_by", "") or "")
    if sender and sender.lower() != identity.username.lower():
        book = existing.get("book_name", "Transfer")
        note = (request.remarks or "").strip()
        body = f"{book} transfer was {decision.lower()}."
        if note:
            body += f" Remarks: {note}"
        messaging.broadcast_notification(
            [sender], "transfer",
            f"Transfer {decision.lower()}",
            body, "transfers.html"
        )

    database.log_action(identity.username, identity.role, "TRANSFER_" + decision.upper(),
                        "transfer", transfer_id, f"Transfer marked {decision}"
...
def messaging_create_conversation(request: ConversationCreate,
    ...
    # Members must be real, active accounts and inside the caller's school scope.
    known = {u["username"].lower(): u for u in database.get_all_users() if u.get("status") == "Active"}
    creator = known.get(identity.username.lower())
    members = []
    allowed_schools = None if identity.is_super_admin else {str(x) for x in (identity.school_ids or [])}
    for m in request.members:
        u = known.get(m.lower())
        if not u: raise HTTPException(status_code=400, detail=f"Unknown user '{m}'.")
        usid = str(u.get("school_id") or "")
        if allowed_schools is not None and usid not in allowed_schools:
            raise HTTPException(status_code=403, detail="Conversation member is outside your school scope.")
        members.append(u["username"])
    requested_school = str(request.school_id or "")
    if identity.is_super_admin:
        school_id = requested_school
    else:
        if requested_school and requested_school not in allowed_schools:
            raise HTTPException(status_code=403, detail="Conversation school is outside your scope.")
        school_id = requested_school or str((creator or {}).get("school_id") or "")
    try:
        conv = messaging.create_conversation(ctype, request.title.strip(), members,
                                             identity.username, school_id)
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))
...
def messaging_add_members(conversation_id: str, request: MembersAdd,
    ...
    if conv.get("type") == messaging.DM:
        raise HTTPException(status_code=400, detail="Members cannot be added to a direct message.")
    known = {u["username"].lower(): u for u in database.get_all_users() if u.get("status") == "Active"}
    allowed = identity.school_ids
    checked = []
    for name in request.members:
        u = known.get(str(name).lower())
        if not u: raise HTTPException(status_code=400, detail=f"Unknown user '{name}'.")
        if allowed is not None and str(u.get("school_id") or "") not in {str(x) for x in allowed}:
            raise HTTPException(status_code=403, detail="That user is outside your school scope.")
        checked.append(u["username"])
    try:
        return {"status": "success", "data": messaging.add_members(conversation_id, checked)}
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))
...
def messaging_send(conversation_id: str, request: MessageSend,
    ...
    if conv.get("type") != messaging.ANNOUNCEMENT:
        # Private messages must arrive as E2EE envelopes. This prevents a
        # modified client/direct API call from bypassing browser encryption.
        try:
            envelope = json.loads(request.body or "")
        except Exception:
            envelope = None
        if not isinstance(envelope, dict) or envelope.get("e2ee") is not True:
            raise HTTPException(status_code=400, detail="Private messages must use end-to-end encryption.")
        if envelope.get("file") and not request.attachment:
            raise HTTPException(status_code=400, detail="Encrypted attachment payload is missing.")
        ...
        try:
            if conv.get("type") != messaging.ANNOUNCEMENT:
                try:
                    ...
                    expected = str((env.get("file") or {}).get("sha256") or "")
                except Exception:
                    expected = ""
                if not expected or base64.b64encode(hashlib.sha256(blob).digest()).decode() != expected:
                    raise HTTPException(status_code=400, detail="Encrypted attachment integrity check failed.")
...
def security_active_sessions(admin: Identity = Depends(require_admin)):
    """Every account currently holding an active device session."""
    return {"sessions": security_mod.list_device_sessions()}


@router.post("/security/sessions/{username}/release")
def security_release_session(username: str, admin: Identity = Depends(require_admin)):
    """Force-releases a stuck session so the account can sign in again."""
    security_mod.end_session(username)
    database.log_action(admin.username, admin.role, "SESSION_RELEASED", "auth",
                        username, "Administrator released the active device session")
...
def _assert_proc_record_school(identity: "Identity", kind: str, record_id: str) -> Dict[str, Any]:
    """Load a persisted finance record and authorize by its stored school."""
    loaders = {
        "request": lambda: next((r for r in procurement.list_requests("", None) if str(r.get("id")) == str(record_id)), None),
        "order": lambda: procurement.get_order(record_id),
        "grn": lambda: next((r for r in procurement.list_grns("", None) if str(r.get("id")) == str(record_id)), None),
        "invoice": lambda: next((r for r in procurement.list_invoices("", None) if str(r.get("id")) == str(record_id)), None),
        "payment": lambda: next((r for r in procurement.list_payments("", None) if str(r.get("id")) == str(record_id)), None),
        "budget": lambda: next((r for r in procurement.list_budgets("", None) if str(r.get("id")) == str(record_id)), None),
        "credit": lambda: next((r for r in procurement.list_notes("credit", "", None) if str(r.get("id")) == str(record_id)), None),
        "debit": lambda: next((r for r in procurement.list_notes("debit", "", None) if str(r.get("id")) == str(record_id)), None),
    }
    rec = loaders[kind]()
    if not rec:
...
def proc_decide_request(request_id: str, request: RequestDecision,
                        identity: Identity = Depends(require_staff)):
    _assert_proc_record_school(identity, "request", request_id)
    try:
...
def proc_order_status(po_id: str, request: OrderStatus,
                      identity: Identity = Depends(require_staff)):
    _assert_proc_record_school(identity, "order", po_id)
    try:
...
def proc_create_invoice(request: InvoiceCreate, identity: Identity = Depends(require_staff)):
    if request.school_id:
        identity.assert_school(request.school_id)
    if request.po_id:
        po = _assert_proc_record_school(identity, "order", request.po_id)
        if request.school_id and str(po.get("school_id")) != str(request.school_id):
            raise HTTPException(status_code=400, detail="Purchase order and invoice school do not match.")
    if request.grn_id:
        grn = _assert_proc_record_school(identity, "grn", request.grn_id)
        if request.school_id and str(grn.get("school_id")) != str(request.school_id):
            raise HTTPException(status_code=400, detail="GRN and invoice school do not match.")
    try:
...
def proc_create_payment(request: PaymentCreate, identity: Identity = Depends(require_staff)):
    ...
    if request.invoice_id:
        inv = _assert_proc_record_school(identity, "invoice", request.invoice_id)
        if request.school_id and str(inv.get("school_id")) != str(request.school_id):
            raise HTTPException(status_code=400, detail="Invoice and payment school do not match.")
    try:
...
def proc_create_note(kind: str, request: NoteCreate,
    ...
    if request.invoice_id:
        inv = _assert_proc_record_school(identity, "invoice", request.invoice_id)
        if request.school_id and str(inv.get("school_id")) != str(request.school_id):
            raise HTTPException(status_code=400, detail="Invoice and note school do not match.")
    try:
...
def proc_delete_budget(budget_id: str, identity: Identity = Depends(require_staff)):
    _assert_proc_record_school(identity, "budget", budget_id)
