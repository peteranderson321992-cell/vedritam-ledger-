...
const App = (function() {
    ...
    const schools = {
        ...
        async loadSchoolAssignmentOptions() {
            const select = document.getElementById('newSchoolStaff');
            const search = document.getElementById('schoolUserSearch');
            if (!select) return;
            try {
                const data = await apiCall('/schools/assignment-options');
                const users = data.users || [];
                const render = () => {
                    const q = (search?.value || '').toLowerCase().trim();
                    select.innerHTML = '<option value="__ALL__">All Users</option>' + users.filter(u => !q ||
                        String(u.username||'').toLowerCase().includes(q) ||
                        String(u.fullName||'').toLowerCase().includes(q))
                        .map(u => `<option value="${ui.escapeHTML(u.username)}">${ui.escapeHTML(u.username)}${u.fullName ? ' — ' + ui.escapeHTML(u.fullName) : ''}</option>`).join('');
                };
                render();
                if (search) search.oninput = render;
            } catch (e) { if (window.App?.ui) ui.showToast(e.message, 'error'); }
        },
        async submitAddSchool(e) {
            ...
            try {
                ...
                await apiCall('/schools', 'POST', {
                    ...
                    assigned_staff: Array.from(document.getElementById('newSchoolStaff')?.selectedOptions || []).map(o => o.value)
                });
    ...
    const activity = {
        ...
        async init() {
            ...
            ['auditUser', 'auditAction'].forEach(id => {
                const el = document.getElementById(id);
                if (el) {
                    el.addEventListener('input', () => { this.page = 1; this.load(); });
                    if (el.tagName === 'SELECT') el.addEventListener('change', () => { this.page = 1; this.load(); });
                }
            });
            const prev = document.getElementById('auditPrev');
            const next = document.getElementById('auditNext');
            if (prev) prev.addEventListener('click', () => { if (this.page > 1) { this.page--; this.load(); } });
            if (next) next.addEventListener('click', () => { if (this.page * this.limit < this.total) { this.page++; this.load(); } });
            await this.load();
        },
        async load() {
            try {
                const u = encodeURIComponent(document.getElementById('auditUser')?.value || '');
                const a = encodeURIComponent(document.getElementById('auditAction')?.value || '');
                const data = await apiCall(`/audit?limit=${this.limit}&page=${this.page}&username=${u}&action=${a}`);
        ...
        render() {
            ...
            if (count) {
                const first = this.total ? ((this.page - 1) * this.limit + 1) : 0;
                const last = Math.min(this.page * this.limit, this.total);
                count.textContent = this.total ? `${first}–${last} of ${this.total} entries` : '0 entries';
            }
            if (!this.rows.length) { body.innerHTML = '<tr><td colspan="5" style="padding:20px;text-align:center;">No activity recorded yet.</td></tr>'; }
            else body.innerHTML = this.rows.map(r => `<tr>
            ...
            const prev = document.getElementById('auditPrev'), next = document.getElementById('auditNext'), page = document.getElementById('auditPage');
            if (prev) prev.disabled = this.page <= 1;
            if (next) next.disabled = this.page * this.limit >= this.total;
            if (page) page.textContent = `Page ${this.page} of ${Math.max(1, Math.ceil(this.total / this.limit))}`;
        },
        async download() {
            try {
...
// Populate the searchable school-assignment user selector without changing the existing layout.
document.addEventListener('DOMContentLoaded', function () { if (window.App?.schools?.loadSchoolAssignmentOptions) App.schools.loadSchoolAssignmentOptions(); });
