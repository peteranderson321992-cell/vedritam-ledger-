...
def init_db():
    if not os.path.exists(USERS_CSV):
        # Fresh installs use the temporary administrator password requested for
        # this build. BOOTSTRAP_ADMIN_PASSWORD may still be supplied by a host
        # such as Render to override it, but a missing variable must not prevent
        # the application from starting. The password can be changed later from
        # Security > Change Password.
        bootstrap = os.getenv("BOOTSTRAP_ADMIN_PASSWORD") or "admin123"
...
def get_audit_log_page(limit: int = 50, offset: int = 0, username: str = "",
                       action: str = "", allowed_usernames: Optional[List[str]] = None) -> Dict[str, Any]:
    """Authorization-aware pagination: scope and filters are applied before slicing.

    This function deliberately does not call get_audit_log(), because that helper
    is a convenience API with a bounded return size. Pagination must operate on
    the complete authorized history so records cannot disappear between pages.
    """
    rows = [r for r in read_csv(AUDIT_CSV) if not is_messaging_audit_row(r)]
    if allowed_usernames is not None:
        allowed = {str(n).lower() for n in allowed_usernames}
        rows = [r for r in rows if str(r.get("username", "")).lower() in allowed]
    if username:
        rows = [r for r in rows if str(r.get("username", "")).lower() == username.lower()]
    if action:
...
def log_action(username: str, role: str, action: str, entity: str = "", entity_id: str = "", details: str = ""):
    ...
    try:
        ...
        # Audit history is append-only; do not truncate it in application code.
        atomic_csv_write(AUDIT_CSV, AUDIT_HEADERS, rows)
    except Exception as e:
        print(f"[audit] failed to write log entry: {e}")


def get_audit_log(limit: int = 200, username: str = "", action: str = "", school_id: str = "",
                   allowed_usernames: Optional[List[str]] = None) -> List[Dict]:
    rows = [r for r in read_csv(AUDIT_CSV) if not is_messaging_audit_row(r)]
    if allowed_usernames is not None:
        allowed = {str(n).lower() for n in allowed_usernames}
        rows = [r for r in rows if str(r.get("username", "")).lower() in allowed]
    ...
    return rows[:max(1, min(limit, 100000))]
...
def update_user(username: str, role: str = None, status: str = None, full_name: str = None,
    ...
    if school_id is not None:
        if school_id and not _school_name(school_id):
            raise ValueError("Selected school does not exist.")
        old_school = str(target.get("school_id") or "")
        new_school = str(school_id or "")
        target['school_id'] = new_school
        # A school change must immediately remove any legacy assigned_staff
        # membership at the old school so scope cannot survive the move.
        if old_school and old_school != new_school:
            schools = read_csv(SCHOOLS_CSV)
            changed = False
            for school in schools:
                if str(school.get("id")) == old_school:
                    names = [n for n in _staff_list(school.get("assigned_staff", ""))
                             if n.lower() != str(username).lower()]
                    school["assigned_staff"] = _staff_raw(names)
                    changed = True
            if changed:
...
def admin_usernames_for_school(school_id) -> List[str]:
    """Active Admin accounts assigned to a school.

    An Admin is assigned either directly through users.csv school_id or through
    the school's assigned_staff list. Super Admin is intentionally excluded;
    this helper is used for school-to-school transfer routing.
    """
    sid = str(school_id or "")
    if not sid:
        return []
    assigned = set()
    for school in read_csv(SCHOOLS_CSV):
        if str(school.get("id")) == sid:
            assigned = {n.strip().lower() for n in _staff_list(school.get("assigned_staff", "")) if n}
            break
    out = []
    for user in get_all_users():
        ...
        if str(user.get("school_id", "") or "") == sid or uname.lower() in assigned:
            out.append(uname)
...
def add_school(name: str, code: str = "", location: str = "", logo: str = "", address: str = "",
    ...
    known = {u["username"].lower(): u for u in read_csv(USERS_CSV)}
    if any(str(n).strip() == "__ALL__" for n in assigned):
        assigned = [u["username"] for u in known.values()
                    if u.get("status") == "Active" and normalize_role(u.get("role","")) != SUPER_ADMIN]
    cleaned = []
    seen = set()
    for n in assigned:
        key = str(n or "").strip().lower()
        if not key or key in seen:
            continue
        if key not in known:
            raise ValueError(f"User account '{n}' does not exist.")
        if known[key].get("status") != "Active":
            raise ValueError(f"User account '{n}' is not active.")
        if normalize_role(known[key].get("role", "")) == SUPER_ADMIN:
            raise ValueError("The Super Admin cannot be assigned as school staff.")
        seen.add(key)
        cleaned.append(known[key]["username"])
    record = {
        "id": new_id,
        "name": name,
        "code": (code or "").strip(),
        ...
        "status": status,
        "assigned_staff": _staff_raw(cleaned),
...
def update_school(school_id, **fields) -> Dict:
    ...
    if fields.get("assigned_staff") is not None:
        ...
        known = {u["username"].lower(): u for u in read_csv(USERS_CSV)}
        if any(str(n).strip() == "__ALL__" for n in names):
            names = [u["username"] for u in known.values()
                     if u.get("status") == "Active" and normalize_role(u.get("role","")) != SUPER_ADMIN]
        cleaned = []
        seen = set()
        for n in names:
...
def delete_ledger_custom_field(key: str, username: str = "") -> bool:
    """Remove only a field owned by the caller."""
    fields = get_ledger_custom_fields()
    who = str(username or "").lower()
    kept = [f for f in fields if not (f["key"] == str(key) and
                                      str(f.get("created_by", "")).lower() == who)]
