...
(function () {
  ...
  function buildPayload(conv, body, file) {
    return Promise.resolve(file ? readFile(file) : null)
      .then(function (att) {
        if (!isPrivate(conv)) return { body: body, attachment: att };
        if (!window.E2EE || !E2EE.supported()) throw new Error('End-to-end encryption is unavailable in this browser.');
        var rawFile = null;
        if (att && att.data) {
          var bin = atob(att.data), bytes = new Uint8Array(bin.length);
          for (var i = 0; i < bin.length; i++) bytes[i] = bin.charCodeAt(i);
          rawFile = { name: att.name, type: att.type, bytes: bytes.buffer };
        }
        return E2EE.encrypt(conv.members || [], body, rawFile).then(function (enc) {
          var encryptedAtt = null;
          if (enc.attachmentBytes) {
            var bytes = new Uint8Array(enc.attachmentBytes), b = '';
            for (var i = 0; i < bytes.length; i++) b += String.fromCharCode(bytes[i]);
            encryptedAtt = { name: att.name, type: att.type, data: btoa(b) };
          }
          return { body: enc.body, attachment: encryptedAtt };
        });
      });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // While a call is being set up the two browsers swap a lot of small
    // messages: poll quickly so the connection is not left half-finished.
    signalTimer = setInterval(function () {
      pollSignals();
      if (window.VCalls && VCalls.active()) setTimeout(pollSignals, 500);
    }, 1200);
